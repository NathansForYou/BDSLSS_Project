...
Recently, molecualar mechanics have captured the attention of scientists who aim at predicting transition structures in modecular biology. As the name suggests, transitions are movement of a molecule structure upward (or downward) the energy curve. Due to the nature of the molecules, transition structures are more difficult to describe than equilibrium geometrics. Knowing where and when these transitions may occure would help us to better understand the structure and properties of proteins. One use of this is in drug developement industry where a major application of enzymatic transition state information involves the design of transition state analogues that will act as tight-binding inhibitors ~\cite{Schramm}. 
