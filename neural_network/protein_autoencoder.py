...
n_hidden_2 = 227 # 2nd layer num features
n_input = 2676 # protein data input (img shape: 892*3)

X = tf.placeholder("float", [None, n_input])

# testing with different weight initializations
weights = {
    'encoder_h1': tf.Variable(tf.random_normal([n_input, n_hidden_1], mean=0.0, stddev=0.5)),
    'encoder_h2': tf.Variable(tf.random_normal([n_hidden_1, n_hidden_2], mean=0.0, stddev=0.5)),
    'decoder_h1': tf.Variable(tf.random_normal([n_hidden_2, n_hidden_1], mean=0.0, stddev=0.5)),
    'decoder_h2': tf.Variable(tf.random_normal([n_hidden_1, n_input], mean=0.0, stddev=0.5)),
}
biases = {
    'encoder_b1': tf.Variable(tf.random_normal([n_hidden_1], mean=0.0, stddev=0.5)),
    'encoder_b2': tf.Variable(tf.random_normal([n_hidden_2], mean=0.0, stddev=0.5)),
    'decoder_b1': tf.Variable(tf.random_normal([n_hidden_1], mean=0.0, stddev=0.5)),
    'decoder_b2': tf.Variable(tf.random_normal([n_input], mean=0.0, stddev=0.5)),
}
...
# Building the encoder using a sigmoid activation function
def encoder(x):
...
# Another version using rectified linear units for activation
def encoder(x):
